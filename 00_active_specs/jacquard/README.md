...
2. **Scheduler Shuttle Plugin**:
    ...
    * **优先级**: 默认 200 (`preparation` 阶段)，可通过动态编排调整。
    > Scheduler 详细设计参见 [`scheduler-component.md`](scheduler-component.md)
...
4. **Skein Builder Plugin**:
    ...
    * **输入来源**:
        * `plannerContext.weavingGuide` (Planner 产出)
...
7. **Filament Parser Plugin**:
    ...
    * 动作: 读取 `blackboard.expected_structure_registry` 与 `blackboard.parser_hints` 注册合法标签，提取 `<content>` 推送给 UI，提取 `<state_update>` 准备后续处理。
...
11. **[Schema Injector Plugin](schema-injector.md)**:
    ...
    * **动作**: 扫描 Pattern (织谱) 配置和动态协议标签 (`<use_protocol>`)，加载对应的 YAML Schema，直接向已构建 Skein 注入 `instruction` 与 `examples`，并写入 `blackboard.expected_structure_registry`、`blackboard.parser_hints`。
...
**Filament 协议**是 Clotho 与 LLM 之间的边界协议。它不用于 Jacquard 与 Mnemosyne、Presentation 之间的内部对象通信。由于协议规范已收口到单一事实来源，Jacquard 侧文档只保留“如何使用协议”，不再重复定义标签与语法。
...
Filament 协议的完整规范、标签体系、解析流程、版本演进等内容请优先参阅 **[Filament Canonical Spec](../protocols/filament-canonical-spec.md)**，再结合 **[Filament 协议概述](../protocols/filament-protocol-overview.md)** 阅读。
