...
Clotho 旨在重新定义 AI 角色扮演（RPG）的交互体验，构建一个**高性能、跨平台、确定性与沉浸感并存**的次世代客户端。我们致力于解决现有方案（如 SillyTavern）中“逻辑与表现混杂”、“上下文管理混沌”以及“性能随对话长度指数级衰减”的根本性痛点。

### 1.1 解决的根本问题域

1. **性能与跨平台**：摒弃 Web 技术栈，拥抱 **Flutter** 生态。通过原生编译实现高性能渲染，同时确保 Windows、Android 等多端体验的高度一致性，彻底解决长文本渲染卡顿与内存泄漏问题。
...
为了消除“自然语言”与“机器指令”之间的模糊地带，我们将 **Clotho 与 LLM 的边界通信** 统一在 **Filament 协议** 之下。
