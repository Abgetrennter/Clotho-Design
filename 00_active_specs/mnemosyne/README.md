...
4. **Narrative Chain (叙事链) - v1.1 重构**:
    ...
    * **内容**:
        * **Turn Summary (微观)**: 每个回合的摘要，直接存储在 Turn 对象中，用于 RAG 检索。
...
- **Turn Summary**: 每个回合的摘要，由 Post-Flash LLM 生成，用于 RAG 检索
...
### 8.1 Pre-Flash: 聚焦与规划 (Focus & Planning)

Pre-Flash 是 Mnemosyne 在主推理循环之前引入的 **Ingestion Pipeline**，用于为 Main LLM 设定战术目标和聚焦方向。
