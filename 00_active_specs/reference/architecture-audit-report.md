...
**修复建议**:
1. 明确区分 **状态存储 (State Storage)** 和 **状态通知 (State Notification)**:
   - Mnemosyne = 唯一状态存储权威源 (SSOT for State Storage)
   - ClothoNexus = 状态变更通知总线 (State Change Notification Bus)
2. 在 [`architecture-principles.md`](../architecture-principles.md) 中增加状态管理分层说明:
   ```markdown
   ### 2.4 状态管理分层
   
   | 层级 | 负责组件 | 职责 |
   |------|----------|------|
   | 持久化层 | Mnemosyne | 状态存储、快照生成、历史回溯 |
   | 通知层 | ClothoNexus | 状态变更事件广播 |
   | 展示层 | Riverpod | UI 状态投影与缓存 |
   ```
...
**修复建议**:
1. 明确 Inspector 的数据获取路径:
   ```mermaid
   sequenceDiagram
       UI->>Jacquard: 请求 UI Schema
       Jacquard->>Mnemosyne: 查询状态元数据
       Mnemosyne-->>Jacquard: 返回 $meta.ui_schema
       Jacquard-->>UI: 返回 Schema 投影
   ```
2. 在 [`presentation/README.md`](../presentation/README.md) 中增加 **"数据访问边界"** 章节
...
**修复建议**: 在 `reference/` 目录下新增 `testing-strategy.md` 文档
...
**修复建议**: 执行全局术语审查，统一使用 [`metaphor-glossary.md`](../metaphor-glossary.md) 定义的标准术语
