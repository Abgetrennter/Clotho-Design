...
## 1.2 写通道与 Mnemosyne 协调机制 (Write Channel & Mnemosyne Coordination)

**核心原则**: InputDraftController 是 UI 子系统的**唯一写通道**，但严禁直接写入 Mnemosyne。所有状态变更必须通过 Jacquard 编排层验证和应用。
...
sequenceDiagram
    ...
    participant IDC as InputDraftController
    participant J as Jacquard Orchestrator
    participant M as Mnemosyne
    
    Note over UI,M: 写操作流程
    UI->>IDC: 用户提交输入 (文本/Intent)
    IDC->>IDC: 验证输入合法性
    IDC->>J: 提交 Intent (IntentSubmitRequest)
    J->>J: 验证 Intent 格式
    J->>J: 执行插件链 (Planner → Scheduler → ...)
    J->>M: 应用状态变更 (OpLog)
    M->>M: 记录 OpLog + 生成快照
    M-->>J: 确认变更 + 新快照
    J-->>IDC: IntentSubmitResponse (success + stateChanges)
    J-->>IDC: 广播状态更新 (onStateSync Stream)
    IDC->>UI: 清空草稿 + 触发重绘
    
    Note right of IDC: ❌ 禁止：直接调用 Mnemosyne.applyChanges()
    Note right of IDC: ✅ 允许：通过 Jacquard.submitIntent()
```

### 状态变更协调

InputDraftController 不直接感知 Mnemosyne 的状态变更，而是通过 Jacquard 的事件总线接收更新：

```dart
class InputDraftController extends ChangeNotifier {
  final JacquardUIAdapter _jacquardAdapter;
  StreamSubscription? _stateSyncSubscription;
  
  InputDraftController(this._jacquardAdapter);
  
  /// 初始化
  void initialize() {
    // ✅ 正确：订阅 Jacquard 的状态同步事件
    _stateSyncSubscription = _jacquardAdapter.onStateSync().listen((snapshot) {
      // 状态更新由 Jacquard 广播，Controller 仅作为消费者
  ...
  Future<void> submitIntent(Intent intent) async {
    // ✅ 正确：通过 Jacquard 提交 Intent
    final response = await _jacquardAdapter.submitIntent(
      IntentSubmitRequest(
        type: intent.type,
        payload: intent.data,
  ...
  /// ❌ 错误示例：直接操作 Mnemosyne
  Future<void> _wrongSubmitIntent(Intent intent) async {
    // 错误：直接写入 Mnemosyne，绕过 Jacquard 编排层
    // 这会破坏 OpLog 链，导致无法回溯历史
    await Mnemosyne.applyChanges({
      'messages': [intent.toJson()],
    });
    clear();
  }
...
| 操作 | 允许 | 禁止 | 正确做法 |
|------|------|------|---------|
| 提交用户输入 | ✅ | - | 通过 `Jacquard.submitIntent()` |
| 读取状态快照 | ✅ | - | 通过 `Jacquard.onStateSync()` |
| 直接写入 Mnemosyne | - | ❌ | 必须通过 Jacquard 代理 |
| 修改 OpLog | - | ❌ | OpLog 只能由 Jacquard 生成 |
| 跳过验证 | - | ❌ | Intent 必须经 Jacquard 验证 |
