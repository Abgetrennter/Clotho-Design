...
## 1.2 数据访问边界 (Data Access Boundaries)

**核心原则**: Inspector 组件严禁直接访问 Mnemosyne 状态树。所有数据访问必须通过 Jacquard 编排层进行代理。
...
sequenceDiagram
    ...
    Inspector->>J: requestUISchema(path: "character.inventory")
    J->>M: 查询状态元数据 ($meta.ui_schema)
    M-->>J: 返回 Schema + 数据投影
    J->>J: 应用 ACL 过滤
    J-->>Inspector: 返回 Schema 投影
    
    Note right of Inspector: ❌ 禁止：直接读取 Mnemosyne.getState()
    Note right of Inspector: ✅ 允许：通过 Jacquard.requestUISchema()
```

### 接口定义

```dart
/// Inspector 通过 Jacquard 请求 UI Schema
///
/// 正确示例:
/// ```dart
...
///     // ✅ 正确：通过 Jacquard 代理访问
///     final uiSchema = await Jacquard.instance.requestUISchema(path);
///     final data = await Jacquard.instance.requestDataProjection(path);
///     _render(uiSchema, data);
///   }
/// }
/// ```
///
...
///     // ❌ 错误：直接访问 Mnemosyne
///     final state = Mnemosyne.getState(path);
///     final schema = state.meta.uiSchema;
///     _render(schema);
///   }
/// }
/// ```
...
Inspector 不直接订阅 Mnemosyne 状态流，而是通过 Jacquard 的事件总线接收状态更新：
...
class _InspectorState extends State<Inspector> {
  ...
  void initState() {
    ...
    _subscription = Jacquard.instance.onStateSync.listen((snapshot) {
      setState(() {
        _stateTree = snapshot;
      });
    });
