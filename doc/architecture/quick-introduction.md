...
Clotho 是一个**高性能、确定性**的下一代 AI 角色扮演（RPG）客户端，旨在解决现有方案（如 SillyTavern）在逻辑处理、上下文管理和性能上的根本性痛点。
...
|----------|--------------|----------------|
| **性能瓶颈** | Web 技术栈导致长文本渲染卡顿、内存泄漏 | **原生技术栈**：Flutter/Dart，60fps 流畅滚动 |
...
3. **记忆生态 (The Memory - Mnemosyne)**
   ...
   - **特性**: 多维上下文链、VWD（Value with Description）数据模型
   - **能力**: 时空回溯、状态快照、动态补丁 (Patching)

### 关键技术组件

| 组件 | 角色 | 关键特性 |
|------|------|----------|
| **Jacquard** | 编排层核心 | 插件化流水线、Skein 容器、模板渲染 |
| **Mnemosyne** | 数据层核心 | VWD 数据模型、状态快照、模板继承 |
...
### 1. 提示词处理流程
```
用户输入 → Planner 意图规划 → Skein Builder 构建 → Template Renderer 渲染 → Assembler 拼接 → LLM 调用 → Filament 解析 → 状态更新
```
...
### 场景 2: 对话流程
```
用户输入 → 意图分析 → 状态快照 → Prompt 组装 → LLM 生成 → Filament 解析 → 状态更新 → UI 渲染
```

### 场景 3: 状态回溯与分支
```
选择历史消息 → Mnemosyne 重建当时状态 → 继续对话 → 创建新分支 → 独立状态演进
```
