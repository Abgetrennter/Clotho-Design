# 混合资源管理与存储规范 (Hybrid Resource Management & Storage Specification)

**版本**: 1.0.0
**日期**: 2026-01-10
**状态**: Active
**关联文档**:
- `00_active_specs/core/infrastructure-layer.md`
- `00_active_specs/mnemosyne/sqlite-architecture.md`
- `00_active_specs/workflows/character-import-migration.md`

---

## 1. 核心理念与架构 (Core Philosophy & Architecture)

Mnemosyne 的存储架构遵循 **“动静分离，逻辑聚合 (Separate Physically, Aggregate Logically)”** 的核心原则。

### 1.1 三大存储支柱
系统将数据存储划分为三个物理隔离的区域，以满足不同的生命周期和管理需求：

1.  **Library (L2 Static)**: 存放 **织谱 (Patterns)**。类似于“游戏安装目录”或“卡带”。
    *   **特性**: 只读、可替换、版本化、易于分享。
    *   **内容**: 角色卡元数据、默认立绘、背景音乐、世界观设定 (Lorebook)。
2.  **Session Data (L3 Dynamic)**: 存放 **会话 (Sessions)**。类似于“游戏存档”。
    *   **特性**: 读写频繁、私有、依赖 L2 但不包含 L2。
    *   **内容**: 聊天记录、变量状态、用户上传的图片、Schema 快照。
3.  **The Vault (Global Cache)**: 存放 **共享资源 (Blobs)**。类似于“系统动态链接库”或“Steam 公共资源库”。
...
| **背景音乐 (大文件/通用)** | The Vault | `asset://vault/...` 或 HTTP | LRU 缓存清理 |
