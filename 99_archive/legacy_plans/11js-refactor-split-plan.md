...
将 `参考文件/数据库/11.js`（约 2008 行）拆分为按功能分组的模块化结构。
...
- [ ] 剧情推进循环状态 (loopState_ACU)
- [ ] 剧情推进规划阶段防护 (planningGuard_ACU)
- [ ] 剧情推进规划任务中止控制器 (abortController_ACU)
- [ ] 剧情推进防重入锁 (isProcessing_Plot_ACU)
- [ ] 全局设置对象 (settings_ACU)
- [ ] 表格保留键集合 (SHEET_KEEP_KEYS_ACU)
...
1. **不保证能正常运行**：拆分后的代码可能缺少某些依赖或初始化顺序问题，这是正常的，因为目标是读懂代码结构。

2. **命名约定**：所有函数和变量保持 `_ACU` 后缀，以标识来源。

3. **依赖处理**：某些模块之间存在循环依赖，需要通过主入口文件协调初始化顺序。

4. **全局变量**：部分全局变量（如 `settings_ACU`, `currentJsonTableData_ACU`）需要通过导出/导入机制在各模块间共享。
